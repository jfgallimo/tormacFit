...
class Dream():
    ...
    def __init__(self, model, variables=None, nseedchains=None, nCR=3, adapt_crossover=True, adapt_gamma=False,
                 crossover_burnin=None, DEpairs=1, lamb=.05, zeta=1e-12, history_thin=1, snooker=.10,
        ...
        try:
            ...
            if self.multitry > 1:
                ...
                if run_snooker:
                    ...
                    # hack
                    mr = T * (q_loglike_noT - self.last_like) + (q_prior - self.last_prior) + (snooker_logp_prop - snooker_current_logp)
                    #q_new = metrop_select(np.nan_to_num(total_proposed_logp - total_old_logp), q, q0)
                    q_new = metrop_select(mr, q, q0)
                else:
                    #q_new = metrop_select(np.nan_to_num(q_logp) - np.nan_to_num(self.last_logp), q, q0)
                    # hack
                    mr = T * (q_loglike_noT - self.last_like) + (q_prior - self.last_prior)
                    q_new = metrop_select(mr, q, q0)
            ...
            if self.iter % self.history_thin == 0:
                '''
                print('Current iter = ', self.iter)
                print('Current thinning = ', self.history_thin)
                '''
                with Dream_shared_vars.history.get_lock() and Dream_shared_vars.count.get_lock():
