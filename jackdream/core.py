...
def run_dream(parameters, likelihood, nchains=5, niterations=50000, start=None, restart=False, verbose=True, nverbose=10, tempering=False, mp_context=None, beta=1, **kwargs):
    ...
    try:
        if tempering:
            ...
            if type(start) is list:
                args = zip([step_instance]*nchains, [niterations]*nchains, start, [verbose]*nchains, [nverbose]*nchains, [step_instance.history_thin]*nchains, [beta]*nchains)

            else:
                args = list(zip([step_instance]*nchains, [niterations]*nchains, [start]*nchains, [verbose]*nchains, [nverbose]*nchains, [step_instance.history_thin]*nchains, [beta]*nchains))
            print('args = ', args)
...
def _sample_dream(args):
    ...
    try: 
        ...
        # hack
        sampled_params = np.empty((iterations // thin, dream_instance.total_var_dimension))
        log_ps = np.empty((iterations //thin, 1))
        q0 = start
        naccepts = 0
        naccepts100win = 0
        for iteration in range(iterations):
            ...
            old_params = q0
            # hack
            _sp, _lprior, _llike = step_fxn(q0, T=beta)
            #sampled_params[iteration], log_prior , log_like = step_fxn(q0)
            #log_ps[iteration] = log_like + log_prior
            # hack
            if iteration % thin == 0:
                '''
                print('iteration // thin = ', iteration // thin)
                print('_sp = ', _sp[0:10])
                print('_lprior = ', _lprior)
                print('_llike = ', _llike)
                '''
                sampled_params[iteration // thin] = _sp
                log_ps[iteration // thin] = _lprior + _llike
            if iteration % nverbose == 0:
                print('log_ps = ', log_ps[iteration // thin])
            # hack
            q0 = _sp
...
def _setup_mp_dream_pool(nchains, niterations, step_instance, start_pt=None, mp_context=None):
    ...
    if step_instance.history_file != False:
        ...
        if niterations < step_instance.history_thin:
            ...
            print('4 nchains = ', nchains)
            print('4 niterations = ', niterations)
            print('4 history_thin = ', step_instance.history_thin)
            print('4 total_var_dimension = ', step_instance.total_var_dimension)
            print('4 nseedchains = ', step_instance.nseedchains)
            print('4 arr_dim = ', arr_dim)
